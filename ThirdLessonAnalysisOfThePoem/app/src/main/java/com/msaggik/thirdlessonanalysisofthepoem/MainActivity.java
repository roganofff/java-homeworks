...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        output.setText(sortWords(poemArray));
    }

    private String sortWords(String[] wordArray) {
        String wordResult = "";
        String sortedWords = "Отсортированные слова: ";

        Arrays.sort(wordArray);

        for (String word : wordArray) {
            wordResult = wordResult.concat(String.format("%s, ", word));
        }

        wordResult = wordResult.substring(0, wordResult.length() - 2);

        return sortedWords + wordResult;
    }
